...
func (ws *WasmSnapshotter) SnapshotExtension(height uint64, payloadWriter snapshot.ExtensionPayloadWriter) error {
	...
	ctx := sdk.NewContext(cacheMS, map[string]storetypes.ArchiveKVStore{}, tmproto.Header{}, false, log.NewNopLogger())
...
func (ws *WasmSnapshotter) processAllItems(
	...
	ctx := sdk.NewContext(ws.cms, map[string]storetypes.ArchiveKVStore{}, tmproto.Header{Height: int64(height)}, false, log.NewNopLogger())
	for {
